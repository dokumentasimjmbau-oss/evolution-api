...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(
    ...
    // Newsletter/Channel bypass
    if (isJidNewsletter(number)) {
      const sender = number.toLowerCase();
      this.logger.verbose(`Sending message to newsletter ${sender}`);
      try {
        const messageSent = await this.sendMessage(sender, message, [], false, undefined);
        const messageRaw: any = {
          key: { fromMe: true, id: messageSent?.key?.id || v4(), remoteJid: sender },
          pushName: await this.getProfileName(),
          status: 'PENDING',
